...
const YLDateUtil = {
  ...
  isTodayHour(date1 = 0, date2 = 0) {
    ...
    if (date1Map.year === date2Map.year && date1Map.month === date2Map.month && date1Map.day === date2Map.day && date1Map.hour === date2Map.hour) {
      return true;
    }
    return false;
  },
  ...
  formatDateString: function (time, format = 'YY-MM-DD hh:mm:ss') {
    ...
    const newTime = format.replace(/YY/g, year).replace(/MM/g, preArr[month] || month).replace(/DD/g, preArr[day] || day).replace(/hh/g, preArr[hour] || hour).replace(/mm/g, preArr[min] || min).replace(/ss/g, preArr[sec] || sec);
...
//# sourceMappingURL=RXDateUtil.js.map